...
class Gestor_lista(Queue):
    ...
    def append(self, item): #Menor a Mayor
        self.A.append(item)
...
class Gestor_lista_subestimacion(Queue):
    """Coste ordenado"""
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A= sorted(self.A, key= lambda x: (x.path_cost + self.B.h(x)))#reverser
